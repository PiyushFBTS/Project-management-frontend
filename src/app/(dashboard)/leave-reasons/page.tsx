...
import { leaveReasonsApi } from '@/lib/api/leave-reasons';
...
export default function LeaveReasonsPage() {
  ...
  const [editing, setEditing] = useState<LeaveReason | null>(null);

  const { data, isLoading } = useQuery({
    queryKey: ['leave-reasons', search, isEmployee],
    queryFn: () =>
      (isEmployee
        ? leaveReasonsApi.employeeGetAll({ search, limit: 100 })
        : leaveReasonsApi.getAll({ search, limit: 100 })
      ).then((r) => r.data.data),
  });
  ...
  const createMutation = useMutation({
    mutationFn: (dto: FormValues) =>
      isEmployee ? leaveReasonsApi.employeeCreate(dto) : leaveReasonsApi.create(dto),
    onMutate: () => ({ id: toast.loading('Creating leave reason...') }),
    onSuccess: (res, _, ctx) => {
      qc.invalidateQueries({ queryKey: ['leave-reasons'] });
      toast.success(`"${res.data.data.reasonName}" created`, { id: ctx?.id });
      setOpen(false);
      reset();
    },
    onError: (e: any, _, ctx) => toast.error(e?.response?.data?.message ?? 'Failed to create leave reason', { id: ctx?.id }),
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, dto }: { id: number; dto: Partial<FormValues> }) =>
      isEmployee ? leaveReasonsApi.employeeUpdate(id, dto) : leaveReasonsApi.update(id, dto),
    onMutate: () => ({ id: toast.loading('Updating leave reason...') }),
    onSuccess: (res, _, ctx) => {
      qc.invalidateQueries({ queryKey: ['leave-reasons'] });
    ...
    onError: (e: any, _, ctx) => toast.error(e?.response?.data?.message ?? 'Failed to update leave reason', { id: ctx?.id }),
  });

  const deleteMutation = useMutation({
    mutationFn: ({ id }: { id: number; name: string }) =>
      isEmployee ? leaveReasonsApi.employeeRemove(id) : leaveReasonsApi.remove(id),
    onMutate: ({ name }) => ({ id: toast.loading(`Deactivating "${name}"...`) }),
    onSuccess: (_, { name }, ctx) => {
      qc.invalidateQueries({ queryKey: ['leave-reasons'] });
      toast.success(`"${name}" deactivated`, { id: ctx?.id });
    },
    onError: (e: any, _, ctx) => toast.error(e?.response?.data?.message ?? 'Failed to deactivate leave reason', { id: ctx?.id }),
  });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative overflow-hidden rounded-2xl shadow-lg">
        ...
        <div className="relative px-6 py-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-white">Leave Reasons</h1>
              <p className="text-sm text-white/60">Leave reason categories</p>
            </div>
          </div>
          {canManage && (
            <Button size="sm" onClick={openCreate} className="bg-white/20 backdrop-blur-sm text-white hover:bg-white/30 border-0 shadow-lg">
              <Plus className="mr-1.5 h-4 w-4" /> New Leave Reason
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="relative w-full sm:max-w-xs">
        ...
        <Input
          placeholder="Search leave reasons..."
          value={search}
      ...
      {canManage && (
        <Dialog open={open} onOpenChange={(v) => { if (!v) { setOpen(false); setEditing(null); } }}>
          <DialogContent className="max-w-sm overflow-hidden">
            ...
            <DialogHeader><DialogTitle className="flex items-center gap-2"><FileText className="h-4 w-4 text-orange-500" />{editing ? 'Edit Leave Reason' : 'New Leave Reason'}</DialogTitle></DialogHeader>
